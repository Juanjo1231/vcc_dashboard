...
MonitorApp.prototype.updateRequirements = function() {
  let tableId = "1nN0YhREhJYGQCyTc9UjC1WmxZUF8cK3IFfcikYqw";
  let query = "https://www.googleapis.com/fusiontables/v2/query?sql=SELECT * FROM "
            + tableId
            + "&alt=csv&key=AIzaSyD0obWn8dQo74pIEKU-7veW4O_pSYpUNug";
...
MonitorApp.prototype.getRequirements = function() {
  ...
  // File Rows/Lines
  for(let a = 1; a < raw_data.length; a++) {
    let row = raw_data[a];
...
MonitorApp.prototype.updateStats = function() {
  ...
  //let team  = row[3].textContent.trim();
    ...
    let state_match = state.match("(Grocery Feedback)|(Grocery Email)|(Grocery Chat)");
    let state_key   = state_match
                    ? state_match[0].trim().toLowerCase().replace(/\s/g, "_")
                    : "grocery_phone";

    if(todayReqs[state_key]) {
      for(key in this.states) {
        let exp    = `${key}`;       // ^Available
        let regExp = new RegExp(exp); // /^Available/
        let stat = this.states[key];

        if(state.match(regExp) && state_key === "grocery_phone")
        {
...
/*
* Update cards with the new requirements.
*/
